...
import { Search, Plus, Building2, Phone, Mail, MapPin, Eye, Edit, Trash2, FileText, TrendingUp, DollarSign } from 'lucide-react';
...
const Proveedores = () => {
  ...
  const gerentesFiltrados = gerentesList.filter(gerente => gerente.nombre.toLowerCase().includes(busqueda.toLowerCase()) || gerente.rif.toLowerCase().includes(busqueda.toLowerCase()) || gerente.categoria.toLowerCase().includes(busqueda.toLowerCase()));
  ...
  const guardarGerenteEditado = (gerenteEditado: any) => {
    const nuevosGerentes = gerentesList.map(gerente => gerente.id === gerenteEditado.id ? gerenteEditado : gerente);
  ...
  return <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button className="bg-blue-600 hover:bg-blue-700" onClick={() => setModalNuevo(true)}>
      ...
      <Card>
        <CardContent className="p-6">
          <div className="relative">
            ...
            <Input placeholder="Buscar por nombre, RIF o categoría..." value={busqueda} onChange={e => setBusqueda(e.target.value)} className="pl-10" />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {gerentesFiltrados.map(gerente => <Card key={gerente.id} className="hover:shadow-lg transition-shadow">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2 pt-4">
                <Button variant="outline" size="sm" className="flex-1" onClick={() => abrirModalVerDetalles(gerente)}>
                  <Eye className="w-4 h-4 mr-2" />
                  Ver Detalles
                </Button>
                <Button variant="outline" size="sm" className="flex-1" onClick={() => abrirModalEditar(gerente)}>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        {gerente.facturas > 0 && <span className="block mt-2 text-red-600 font-medium">
      ...
      {gerentesFiltrados.length === 0 && <Card>
      ...
      {/* Modales */}
      <VerDetallesGerenteModal isOpen={modalVerDetalles} onClose={() => setModalVerDetalles(false)} gerente={gerenteSeleccionado} />

      <EditarGerenteModal isOpen={modalEditar} onClose={() => setModalEditar(false)} gerente={gerenteSeleccionado} onSave={guardarGerenteEditado} />

      <NuevoGerenteModal isOpen={modalNuevo} onClose={() => setModalNuevo(false)} onSave={guardarNuevoGerente} />
    </div>;
};
